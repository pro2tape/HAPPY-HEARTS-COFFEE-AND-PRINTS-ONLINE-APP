...
import { SettingsIcon, ClipboardListIcon } from './Icons';
...
const Admin: React.FC = () => {
  ...
  return (
    <>
      <div className="bg-slate-100 min-h-screen font-sans">
        ...
        <main className="container mx-auto p-4 sm:p-6 lg:p-8">
          <section className="bg-white p-6 rounded-lg shadow-md mb-8 non-printable border border-slate-200">
              <h2 className="text-xl font-semibold text-slate-700 mb-4">Actions</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <a
                    href="#/staff"
                    className="flex flex-col items-center justify-center bg-green-600 text-white font-bold py-6 px-6 rounded-lg hover:bg-green-700 transition-colors text-xl"
                >
                    Point of Sale (Staff View)
                </a>
                <button
                    onClick={() => setIsMenuManagerOpen(true)}
                    className="flex flex-col items-center justify-center bg-amber-500 text-white font-bold py-6 px-6 rounded-lg hover:bg-amber-600 transition-colors text-xl"
                >
                    Manage Menu Items
                </button>
              </div>
      ...
      {isMenuManagerOpen && <MenuManager isOpen={isMenuManagerOpen} onClose={() => setIsMenuManagerOpen(false)} currentMenuData={menuData} />}
    </>
  );
};
